...
bool CInputStreamPVRMedia::OpenPVRStream()
{
  ...
  if (!mediaTag)
    CLog::Log(LOGERROR, "CInputStreamPVRMedia - %s - unable to obtain mediaTag instance for mediaTag %s", __FUNCTION__, m_item.GetPath().c_str());

  if (mediaTag && m_client && (m_client->OpenMediaStream(mediaTag) == PVR_ERROR_NO_ERROR))
  {
    CLog::Log(LOGDEBUG, "CInputStreamPVRMedia - %s - opened mediaTag stream %s", __FUNCTION__, m_item.GetPath().c_str());
    return true;
  }
...
void CInputStreamPVRMedia::ClosePVRStream()
{
  ...
  {
    CLog::Log(LOGDEBUG, "CInputStreamPVRMedia - %s - closed mediaTag stream %s", __FUNCTION__, m_item.GetPath().c_str());
  }
}
