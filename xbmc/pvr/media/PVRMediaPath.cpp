...
const std::string CPVRMediaPath::PATH_ACTIVE_TV_MEDIA = "pvr://media/tv/active/";
const std::string CPVRMediaPath::PATH_ACTIVE_RADIO_MEDIA = "pvr://media/radio/active/";
const std::string CPVRMediaPath::PATH_DELETED_TV_MEDIA = "pvr://media/tv/deleted/";
const std::string CPVRMediaPath::PATH_DELETED_RADIO_MEDIA = "pvr://media/radio/deleted/";
...
CPVRMediaPath::CPVRMediaPath(const std::string& strPath)
{
  ...
  m_bValid = ((segments.size() >= 4) && // at least pvr://media/[tv|radio]/[active|deleted]
...
CPVRMediaPath::CPVRMediaPath(bool bDeleted, bool bRadio)
  : m_bValid(true),
    m_bRoot(true),
    m_bActive(!bDeleted),
    m_bRadio(bRadio),
    m_path(StringUtils::Format(
        "pvr://media/%s/%s/", bRadio ? "radio" : "tv", bDeleted ? "deleted" : "active"))
{
}

CPVRMediaPath::CPVRMediaPath(bool bDeleted,
                             bool bRadio,
  ...
  : m_bValid(true), m_bRoot(false), m_bActive(!bDeleted), m_bRadio(bRadio)
{
  ...
  m_directoryPath =
      StringUtils::Format("%s%s%s%s%s", strDirectoryN.c_str(), strTitleN.c_str(),
                          strSeasonEpisodeN.c_str(), strYearN.c_str(), strSubtitleN.c_str());
  m_params = StringUtils::Format(", TV%s, %s, %s.pvr", strChannelNameN.c_str(),
                                 mediaTagTime.GetAsSaveString().c_str(), strId.c_str());
  m_path = StringUtils::Format("pvr://media/%s/%s/%s%s", bRadio ? "radio" : "tv",
...
size_t CPVRMediaPath::GetDirectoryPathPosition() const
{
  ...
  {
    if (m_bRadio)
      return PATH_ACTIVE_RADIO_MEDIA.size();
    else
      return PATH_ACTIVE_TV_MEDIA.size();
  }
  else
  {
    if (m_bRadio)
      return PATH_DELETED_RADIO_MEDIA.size();
    else
      return PATH_DELETED_TV_MEDIA.size();
  }
  // unreachable
}
