...
void CGUIDialogPVRMediaTagSettings::InitializeSettings()
{
  ...
  // Name
  setting = AddEdit(group, SETTING_MEDIA_TAG_NAME, 19075, SettingLevel::Basic, m_strTitle);
  setting->SetEnabled(client && client->GetClientCapabilities().SupportsMediaRename());
...
bool CGUIDialogPVRMediaTagSettings::CanEditMediaTag(const CFileItem& item)
{
  ...
  return capabilities.SupportsMediaRename() || capabilities.SupportsMediaPlayCount() ||
         capabilities.SupportsMediaLifetimeChange();
}
