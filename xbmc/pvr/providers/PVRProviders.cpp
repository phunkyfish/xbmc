...
bool CPVRProvidersContainer::UpdateFromClient(const std::shared_ptr<CPVRProvider>& provider)
{
  ...
  {
    provider->SetProviderId(++m_iLastId);
    ...
    const std::shared_ptr<CPVRProvider>& newProvider, ProviderUpdateMode updateMode)
{
  ...
  else
  {
    ...
    if (newProvider->GetProviderId() > m_iLastId)
      m_iLastId = newProvider->GetProviderId();
    ...
    const std::shared_ptr<CPVRProvider>& newProvider, ProviderUpdateMode updateMode)
{
  ...
  {
    newProvider->SetProviderId(++m_iLastId);
    InsertEntry(newProvider, updateMode);
...
{
  std::shared_ptr<CPVRProvider> item;
  CSingleLock lock(m_critSection);
  for (const auto& provider : m_providers)
  {
    if (provider->GetProviderId() == iProviderId)
      return provider;
  }
