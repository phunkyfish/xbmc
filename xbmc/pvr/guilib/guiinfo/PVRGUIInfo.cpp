...
{
  int toggleIntervalMs = CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_iPVRInfoToggleInterval;
...
bool CPVRGUIInfo::GetListItemAndPlayerLabel(const CFileItem* item, const CGUIInfo& info, std::string& strValue) const
{
  ...
  if (mediaTag)
  {
    ...
    switch (info.m_info)
    {
      ...
      case VIDEOPLAYER_CHANNEL_NAME:
      case LISTITEM_CHANNEL_NAME:
        if (mediaTag->m_strProviderName.empty())
        {
          const auto& provider = mediaTag->GetProvider();
          strValue = provider->GetProviderName();
        }
        else
        {
      ...
      case LISTITEM_ICON:
        {
          auto provider = mediaTag->GetProvider();
          if (provider->GetIconPath().empty())
          {
            provider = mediaTag->GetDefaultProvider();
            if (!provider->GetIconPath().empty())
            {
